...
const ChubCayRegistration = () => {
  ...
  const getTournamentRecord = async () => {
    try {
      ...
      if (response?.data) {
        const filteredData = tournamentData[0].filter((tournament)=>tournament.tournament_category === "Chub Cay")
        setShowRecords(tournamentData[0].filter((tournament)=>tournament.tournament_category === "Chub Cay"));
      }
    } catch (err) {
