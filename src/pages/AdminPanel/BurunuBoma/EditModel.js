...
import { useUpdateTournamentMutation } from "../../../features/tournaments/api";
...
const EditModal = ({ show, onClose, data, refreshData }) => {
    ...
    const { register, handleSubmit, setValue, formState: { errors } } = useForm({
        ...
        defaultValues: {
            tournament_category: data?.tournament_category || "Chub Cay Classic",
            team_name: data?.team_name || "",
            email: data?.email || "",
            captain_name: data?.captain_name || "",
            boat_name: data?.boat_name || "",
            angler_name: data?.angler_name || "",
            number_of_team_members: data?.number_of_team_members || "",
        }
    });
    ...
    useEffect(() => {
        if (data) {
            setValue("tournament_category", data.tournament_category || "Chub Cay Classic");
            setValue("team_name", data.team_name || "");
            setValue("email", data.email || "");
            setValue("captain_name", data.captain_name || "");
            setValue("boat_name", data.boat_name || "");
            setValue("angler_name", data.angler_name || "");
            setValue("number_of_team_members", data.number_of_team_members || "");
        }
    }, [data, setValue]);
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className=" mb-3">
                        ...
                        <select className="form-select custom-select" {...register("tournament_category")}>
                            <option value="Chub Cay Classic" selected={data?.tournament_category === "Chub Cay Classic"}>
                                Chub Cay Classic
                            </option>
                            <option value="Chub Cay Open" selected={data?.tournament_category === "Chub Cay Open"}>
                                Chub Cay Open
                            </option>
                            <option value="Chub Cay Invitational" selected={data?.tournament_category === "Chub Cay Invitational"}>
